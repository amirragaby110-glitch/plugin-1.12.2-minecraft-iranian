...
/**
 * Iranian Mobs Manager (v5.1 Sound-Only Edition)
 * - 30 Mythical & Historical Iranian Mobs & Bosses
 * - Mobs produce VOCAL SOUNDS ONLY (zero text, zero chat spam, zero hit-blocking ArmorStands)
 * - 100% normal combat hit detection: players can always damage mobs!
 */
public class IranianMobsManager implements Listener {

    private final IranianHardcorePlugin plugin;
    private final Random random = new Random();
    private final Map<UUID, Long> mobSoundCooldown = new HashMap<>();

    public IranianMobsManager(IranianHardcorePlugin plugin) {
        this.plugin = plugin;
        plugin.getServer().getPluginManager().registerEvents(this, plugin);
        startMobSpawnTask();
        plugin.getLogger().info("IranianMobsManager: 30 Iranian mobs vocal sound system initialized!");
    }
    ...
    public LivingEntity spawnCustomMob(Location loc, CustomMobType type) {
        try {
            ...
            // Cinematic audio roar for bosses (Sound only, no text)
            if (type.getHealth() >= 65.0) {
                loc.getWorld().playSound(loc, Sound.ENTITY_ENDERDRAGON_GROWL, 1.2f, 0.65f);
            }
    ...
    public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
        ...
        long last = mobSoundCooldown.getOrDefault(damager.getUniqueId(), 0L);

        // 1. Iranian Custom Mob Vocal Sound
            ...
            try {
                ...
                for (CustomMobType.MobEffect effect : type.getEffects()) {
                    if (random.nextDouble() < effect.getChance()) {
                        // Do NOT give WEAKNESS to player
                        if (effect.getType() != org.bukkit.potion.PotionEffectType.WEAKNESS) {
                ...
                if (now - last > 3000) {
                    mobSoundCooldown.put(damager.getUniqueId(), now);
                    Sound vocalSound = getCustomMobSound(type);
                    float pitch = getCustomMobPitch(type);
                    // PURE AUDIO PLAYBACK - ZERO TEXT, ZERO HITBOX BLOCKING
                    SpeechUtils.playVoice(damager, vocalSound, pitch);
                }
            } catch (Exception ignored) {}
            return;
        }

        // 2. Vanilla mobs vocal sound
        if (now - last > 4000 && random.nextDouble() < 0.40) {
            mobSoundCooldown.put(damager.getUniqueId(), now);
            Sound vocalSound = null;
            float pitch = 1.0f;
        ...
        if (entity.hasMetadata("iranian_mob")) {
            ...
            try {
                ...
                if (entity.getKiller() != null) {
                    entity.getKiller().playSound(entity.getKiller().getLocation(), Sound.UI_TOAST_CHALLENGE_COMPLETE, 1.0f, 1.0f);
                }
            } catch (Exception ignored) {}
            return;
        }

        // 2. Vanilla mob death audio
        if (entity.getKiller() != null && random.nextDouble() < 0.3) {
            SpeechUtils.playVoice(entity, Sound.ENTITY_ZOMBIE_DEATH, 0.9f);
        }
    }
